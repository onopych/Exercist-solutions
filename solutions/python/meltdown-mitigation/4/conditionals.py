...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    is_criticality_balanced_v = temperature < 800 and neutrons_emitted > 500 and (temperature * neutrons_emitted) < 500000

    return is_criticality_balanced_v
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    product_of_temperature_and_neutrons_emitted_per_second = temperature * neutrons_produced_per_second

    if product_of_temperature_and_neutrons_emitted_per_second < ((threshold / 100) * 90):
        return 'LOW'
    if ((threshold / 100) * 90) <=  product_of_temperature_and_neutrons_emitted_per_second <= ((threshold / 100) * 110):
        return 'NORMAL'
    return 'DANGER'